<script>
    ...
    onMount(async () => {
        ...
        searchInput.value = decodeURI($items.currentSearch.replace(/\+/g," "));
    })
...
<form on:submit|preventDefault={handleSubmit} class="relative flex justify-center pt-20 pb-10 px-4 md:px-8 w-full">
    <div class="relative w-full md:w-1/2">
        <input 
            type="search" 
            id="search" 
            class="appearance-none rounded-lg border-gray-200 bg-gray-200 pl-10 pr-8 py-2 w-full placeholder:text-sm border-solid focus:border-gray-300 focus:shadow-sm border-2 focus:outline-none" 
            placeholder="Search" 
